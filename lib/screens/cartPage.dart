            ...
            builder:
              ...
              if (snapshot1.hasData && snapshot1.data!.docs.length > 0) {
                ...
                snapshot1.data!.docs.forEach((item) {
                  ...
                  // Check if item.data() returns a Map and access 'count'
      ...
      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
        if (snapshot.hasData && snapshot.data!.docs.length > 0) {
          ...
          snapshot.data!.docs.forEach((item) {
            // Safely get the data from Firestore document
            var data = item.data() as Map<String, dynamic>;

            // Ensure that 'count' for this item exists, and if not, set it to 0 or another default value
            int? itemCount = count[item.id] ?? 0;
            ...
            _cartItems.add(Cart(
              ...
              itemCount,
              data['item_name'] ??
                  'Unknown', // Default 'Unknown' if item_name doesn't exist
              data['total_qty'] ?? 0, // Default 0 if total_qty doesn't exist
              data['price'] ?? 0.0, // Default 0 if price doesn't exist
            ));
          });
